  ...
  return (
    <AppShell>
        <main className="app-container p-4 sm:p-6 xl:p-10 space-y-8">
          <div className="flex flex-col xl:flex-row xl:justify-between xl:items-end gap-6 rounded-lg border border-border/70 bg-card/65 p-6 sm:p-8 shadow-2xl shadow-black/20">
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              className="space-y-3"
            >
              <p className="text-xs font-black uppercase tracking-[0.28em] text-primary">Dashboard</p>
              <h1 className="text-3xl sm:text-5xl font-black tracking-tight">Welcome, {user?.name.split(' ')[0]}.</h1>
              <p className="max-w-2xl text-sm sm:text-base text-muted-foreground">Track resumes, scan job matches, and generate career assets from one focused workspace.</p>
            </motion.div>
            <div className="flex gap-3 flex-wrap">
               <Link href="/builder/new">
                 <Button className="grad-primary border-none font-black gap-2 rounded-lg h-11 px-5 shadow-lg shadow-primary/20">
                   <Plus className="w-4 h-4" /> New Resume
                 </Button>
               </Link>
               <Link href="/analyzer">
                 <Button variant="outline" className="border-border/70 hover:bg-muted font-bold gap-2 rounded-lg h-11 px-5">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6">
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                {loading ? (
                  Array(2).fill(0).map((_, i) => <Skeleton key={i} className="h-64 rounded-lg bg-card border-border" />)
                ) : resumes.length > 0 ? (
                  resumes.map((resume, idx) => (
                    <ResumeCard key={resume.id} resume={resume} delay={idx * 0.1} onDelete={() => deleteResume(resume.id)} />
                  ))
                ) : (
                  <Card className="lg:col-span-2 min-h-[360px] flex flex-col items-center justify-center p-8 sm:p-12 bg-card/70 border-dashed border-2 border-border/70 rounded-lg shadow-xl">
                    <div className="w-20 h-20 rounded-lg bg-muted flex items-center justify-center mb-6 border border-border">
                       <Plus className="w-10 h-10 text-primary" />
                    </div>
                    <p className="text-foreground font-black text-2xl mb-2">No resumes yet</p>
                    <p className="text-muted-foreground text-sm mb-8 text-center max-w-md">Create your first resume and this area becomes your resume command center with drafts, scores, and quick actions.</p>
                    <Link href="/builder/new">
                      <Button className="grad-primary border-none font-black px-8 rounded-lg shadow-xl shadow-primary/20 h-11">Build My First Resume</Button>
                    </Link>
                  </Card>
                )}
              </div>
...
function StatCard({ icon: Icon, label, value, color }: any) {
  return (
    ...
    >
      <Card className="bg-card/80 border-border/70 shadow-xl shadow-black/10 hover:border-primary/30 transition-all rounded-lg overflow-hidden group">
        <CardContent className="p-6 flex items-center gap-5">
          <div className={cn("p-4 rounded-lg bg-muted/70 group-hover:scale-105 transition-transform duration-300", color)}>
            <Icon className="w-7 h-7" />
          </div>
          <div>
            <p className="text-[11px] text-muted-foreground uppercase font-black tracking-[0.18em] mb-1">{label}</p>
...
function TargetCard({ value, label }: any) {
  return (
    ...
    >
      <Card className="grad-primary text-white overflow-hidden relative border-none shadow-2xl shadow-primary/30 rounded-lg min-h-full">
        <CardContent className="p-6">
          <div className="relative z-10">
            <p className="text-white/70 text-[11px] font-black uppercase tracking-[0.18em] mb-1">{label}</p>
            <p className="text-5xl font-black tracking-tight">{value}</p>
            <div className="mt-4 inline-flex items-center gap-1.5 bg-white/15 px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-normal">
...
function ResumeCard({ resume, delay, onDelete }: any) {
  return (
    <motion.div
      ...
      transition={{ delay }}
    >
      <Card className="group hover:border-primary/50 transition-all duration-300 cursor-pointer bg-card/80 border-border/70 rounded-lg overflow-hidden shadow-xl shadow-black/10">
        ...
        <CardHeader className="pb-4 pt-6 flex flex-row items-start justify-between">
          ...
          <DropdownMenu>
             <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-md hover:bg-muted"><MoreVertical className="w-4 h-4" /></Button>
             </DropdownMenuTrigger>
        ...
        <CardContent>
          ...
          <div className="mt-8 flex gap-3">
            <Link href={`/builder/${resume.id}`} className="flex-1">
              <Button size="sm" className="w-full text-xs font-black uppercase tracking-widest rounded-lg h-10 grad-primary border-none shadow-lg shadow-primary/10">Edit</Button>
            </Link>
            <Link href="/analyzer" className="flex-1">
              <Button variant="outline" size="sm" className="w-full text-xs font-black uppercase tracking-widest rounded-lg h-10 border-border/50 hover:bg-muted">Score</Button>
            </Link>
          </div>
        </CardContent>
      </Card>
  ...
  return (
    <Link href={href}>
      <motion.div whileHover={{ x: 5 }} transition={{ type: 'spring', stiffness: 400 }}>
        <Card className="hover:bg-muted/35 transition-all cursor-pointer group bg-card/80 border-border/70 rounded-lg overflow-hidden">
          <CardContent className="p-5 flex items-center gap-5">
            ...
            <div className="w-8 h-8 rounded-md bg-muted flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
